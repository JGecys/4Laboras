 ...
 *            klasės konstruktorių turi būti paduodamas Comparator<T> interfeisą
...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    public SetADT<E> headSet(E element) {
        headset = new BstSetKTU<>();
        if (this.root.element.equals(element)) {
            headset.root = new BstNode<>(element);
            return headset;
        }
        if (setRecursive(element, this.root.left)) {
            BstNode<E> tmp = new BstNode<>(this.root.element);
            tmp.left = headset.root;
            headset.root = tmp;
        }
        if (setRecursive(element, this.root.right)) {
            BstNode<E> tmp = new BstNode<>(this.root.element);
            tmp.right = headset.root;
            headset.root = tmp;
        }
    ...
    private BstSetKTU<E> headset = null;
    ...
    private boolean setRecursive(E element, BstNode<E> node) {
        ...
        if (setRecursive( element, node.left)) {
            BstNode<E> tmp = new BstNode<>(node.element);
            tmp.left = headset.root;
            headset.root = tmp;
            return true;
        }
    ...
    private BstSetKTU<E>.BstNode<E> find(E element){
        IteratorKTU it = (IteratorKTU) iterator();
        while(it.hasNext()){
    ...
    public SetADT<E> subSet(E element1, E element2) {
        BstSetKTU<E> tailSet = (BstSetKTU<E>) tailSet(element1);
        headset = new BstSetKTU<>();
        ...
        if (setRecursive(element2, tailSet.root.left)) {
            BstNode<E> tmp = new BstNode<>(tailSet.root.element);
            tmp.left = headset.root;
            headset.root = tmp;
        }
